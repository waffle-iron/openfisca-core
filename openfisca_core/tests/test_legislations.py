...
def test_get_node():
    taux = legislations.get_node(tax_benefit_system.get_legislation(), 'csg.activite.deductible.taux')
...
def test_get_node_with_path_too_long():
    legislations.get_node(tax_benefit_system.get_legislation(), 'csg.activite.deductible.abattement.x')


@raises(ValueError)
def test_get_node_with_wrong_path():
    legislations.get_node(tax_benefit_system.get_legislation(), 'xx')


def test_at_instant():
    abattement = legislations.get_node(tax_benefit_system.get_legislation(), 'csg.activite.deductible.abattement')
    abattement_at_instant = legislations.at_instant(abattement, '2005-06-01')
